window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function ClientBindCard(props) {
      ...
      var pubState = React.useState(null);
      var pub = pubState[0];
      var setPub = pubState[1];
      ...
      function fetchPub() {
        setPub({ loading: true });
        fetch("/client-bind/pubkeys").then(function (r) { return r.json(); }).then(function (out) {
          setPub(out.error ? { error: out.error } : { keys: out.keys || [] });
        }).catch(function (e) { setPub({ error: String(e) }); });
      }
      function copy(text) {
      ...
      children.push(React.createElement("div", { key: "pub", style: { marginTop: 12 } },
        React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 8 } },
          React.createElement("span", { style: { fontSize: 12, color: "var(--dsw-alias-label-secondary)" } }, "~/.ssh 已有公钥（追加到目标机器的 authorized_keys 即可免密登陆）"),
          React.createElement("span", { style: { flex: 1 } }),
          React.createElement("button", { style: btnStyle, onClick: fetchPub }, pub && pub.loading ? "读取中…" : "🔑 读取公钥"))));
      if (pub && pub.error) {
        children.push(React.createElement("div", { key: "puberr", style: { fontSize: 12, color: "var(--dsw-alias-label-error)", marginTop: 6 } }, pub.error));
      }
      if (pub && Array.isArray(pub.keys)) {
        if (!pub.keys.length) {
          children.push(React.createElement("div", { key: "pubnone", style: Object.assign({}, muted, { marginTop: 6 }) }, "~/.ssh 下没有 .pub 公钥文件"));
        }
        pub.keys.forEach(function (k, i) {
          children.push(React.createElement("div", { key: "pub" + i, style: Object.assign({}, cardStyle, { marginTop: 8 }) },
            React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 8, marginBottom: 4 } },
              React.createElement("span", { style: Object.assign({}, muted, { fontFamily: "monospace" }) }, k.file),
              React.createElement("span", { style: { flex: 1 } }),
              React.createElement("button", { style: btnStyle, onClick: function () { copy(k.publicKey); } }, "复制公钥")),
            React.createElement("textarea", {
              readOnly: true,
              style: Object.assign({}, inputStyle, { minHeight: 52, fontFamily: "monospace", fontSize: 12, resize: "vertical" }),
              value: k.publicKey,
